...
- [Запросы после шардирования (ЛР №6)](#запросы-после-шардирования-лр-6)
...
│   └── lab-06/                       # Скрипты экспериментов ЛР №6 (запросы после шардирования)
│
├── docs/
...
│   ├── lab-06-distributed-queries.md # Отчёт по ЛР №6
...
## Запросы после шардирования (ЛР №6)

Отчёт — [docs/lab-06-distributed-queries.md](docs/lab-06-distributed-queries.md); он же по ГОСТ 7.32-2017 — [docs/lab-06-report.docx](docs/lab-06-report.docx) ([PDF](docs/lab-06-report.pdf)).

![Запись терминала: агрегация и ORDER BY + LIMIT по шардам, отказ шарда](docs/lab-06/demo.gif)

| Изменение | Зачем |
|---|---|
| `DistributedTaskQueryService`, `DistributedMerge` | scatter-gather по шардам: COUNT/AVG через суммы и количества, слияние top-N для ORDER BY + LIMIT, JOIN задач с проектами на стороне сервиса |
| `ShardUnavailableException` → HTTP 503 `SHARD_UNAVAILABLE`, `Timeout=3` в строках подключения к шардам, сброс пула при оборванном соединении | отказ шарда даёт быструю понятную ошибку, остальной сервис работает |
| `allowPartial=true` у распределённых запросов | неполный результат доступных шардов с явной пометкой `partial` |
| `GET /api/shards/queries/stats`, `…/newest`, `…/users/{userId}/tasks-with-projects`, `…/health`, `POST …/workload` | распределённые запросы, состояние шардов, генератор нагрузки для опыта с горячим шардом (роль Admin) |

Измерения: объединённые COUNT и AVG по трём шардам совпали с основной базой; JOIN задач с проектами через шард и основную базу — 24 мс против 2 мс в одной базе; один шард содержит лишь 2–5 задач настоящей первой страницы; при отказе Shard 2 недоступна треть задач, остальной сервис отвечает 200; популярный пользователь с 75 % запросов даёт своему шарду 82 % нагрузки при 32 % данных.

Повторить эксперименты (нужны `curl`, `jq` и загруженные шарды из ЛР №5):

```bash
docker compose up -d
bash scripts/lab-06/run-all.sh                            # разделы 2–7: запросы, отказ шарда, горячий шард
powershell -File scripts/lab-06/demo-distributed.ps1      # демонстрация: слияние результатов и отказ шарда
vhs scripts/lab-06/demo.tape                              # запись терминала
```
...
Логику партиций (ЛР №3) проверяют тесты в `tests/TaskManager.Tests/Partitioning`: планировщик партиций, разбор границ из `pg_get_expr`, политика алертов и сервис целиком на подменённых часах. Разделение чтения и записи между Primary и Replica (ЛР №4) проверяют тесты `TaskServiceTests`. Router'ы шардирования и расчёт переноса данных (ЛР №5) проверяют тесты в `tests/TaskManager.Tests/Sharding`. Объединение результатов распределённых запросов и поведение при отказе шарда (ЛР №6) проверяют тесты `DistributedQueryTests`. Всего в проекте 110 тестов.
